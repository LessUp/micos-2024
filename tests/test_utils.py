...
def test_get_full_run_defaults_merges_analysis_and_databases_config(
    ...
    assert defaults == {
        "input_dir": str((tmp_path / "config" / "data" / "raw_input").resolve()),
        "results_dir": str((tmp_path / "config" / "results").resolve()),
        "threads": 24,
