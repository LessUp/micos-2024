...
class AmpliconAnalyzer:
    ...
    def quality_control_16s(self, input_dir, output_dir):
        ...
        try:
            ...
            result = subprocess.run(
                ["qiime", "--version"], capture_output=True, text=True
            )
            ...
            import_cmd = [
                ...
                str(manifest_file),
                "--output-path",
                str(output_dir / "demux-paired-end.qza"),
                "--input-format",
            ...
            demux_viz_cmd = [
                ...
                str(output_dir / "demux-paired-end.qza"),
                "--o-visualization",
                str(output_dir / "demux-paired-end.qzv"),
            ]
    ...
    def create_manifest_file(self, input_dir):
        ...
        for sample_id in samples:
            ...
            if r1_file.exists() and r2_file.exists():
                manifest_data.append(
                    f"{sample_id}\t{r1_file.absolute()}\t{r2_file.absolute()}"
                )
    ...
    def denoise_sequences(self, input_qza, output_dir):
        ...
        try:
            ...
            dada2_cmd = [
                ...
                "150",
                "--o-table",
                str(output_dir / "table.qza"),
                "--o-representative-sequences",
                str(output_dir / "rep-seqs.qza"),
                "--o-denoising-stats",
                str(output_dir / "denoising-stats.qza"),
            ]
    ...
    def diversity_analysis(
        self, table_qza, rep_seqs_qza, taxonomy_qza, metadata_file, output_dir
    ):
        """多样性分析"""
        logger.info("进行多样性分析...")
        ...
        try:
            ...
            phylogeny_cmd = [
                ...
                str(rep_seqs_qza),
                "--o-alignment",
                str(output_dir / "aligned-rep-seqs.qza"),
                "--o-masked-alignment",
                str(output_dir / "masked-aligned-rep-seqs.qza"),
                "--o-tree",
                str(output_dir / "unrooted-tree.qza"),
                "--o-rooted-tree",
                str(output_dir / "rooted-tree.qza"),
            ]
            ...
            core_metrics_cmd = [
                ...
                str(metadata_file),
                "--output-dir",
                str(output_dir / "core-metrics-results"),
            ]
    ...
    def generate_taxa_barplot(
        self, table_qza, taxonomy_qza, metadata_file, output_file
    ):
        """生成分类学条形图"""
    ...
    if analyzer.quality_control_16s(args.input, analyzer.qc_dir):
        ...
        if analyzer.denoise_sequences(demux_qza, analyzer.otu_dir):
            ...
            if args.classifier and Path(args.classifier).exists():
                ...
                if analyzer.assign_taxonomy(
                    rep_seqs_qza, args.classifier, analyzer.taxonomy_dir
                ):
                    ...
                    analyzer.diversity_analysis(
                        ...
                        taxonomy_qza,
                        args.metadata,
                        analyzer.diversity_dir,
                    )

                    # 5. 生成分类学条形图
                    barplot_file = analyzer.taxonomy_dir / "taxa-bar-plots.qzv"
                    analyzer.generate_taxa_barplot(
                        table_qza, taxonomy_qza, args.metadata, barplot_file
                    )
